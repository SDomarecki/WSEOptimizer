...
save_dir = 'database/stooq'
os.makedirs(save_dir, exist_ok=True)
for ticker in tickers:
    ticker = ticker.lower()
    if downloaded > companies_to_download:
        break
    # Delete existing file to reload data from stooq
    if os.path.isfile(f'{save_dir}/{ticker}_d.csv'):
        continue
    url = f'{url_base}{ticker}{url_end}'
    filename = wget.download(url, out=save_dir)
    downloaded += 1
    print(f'Downloaded {ticker} [{downloaded}/{companies_to_download}]')
