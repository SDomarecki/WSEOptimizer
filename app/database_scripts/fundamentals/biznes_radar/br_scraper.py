...
class BRScraper:
    ...
    def get_fundamentals(self, company: CompanyDetails) -> pd.DataFrame:
        ...
        fundamentals = pd.concat([df1, df2, df3, df4], axis=1, sort=False).sort_index(
            inplace=False
        )
        return fundamentals

    def get_raw_fundamentals(self, link: str):
        indicators_name_and_locations = {"Sales": 2, "Earnings": 6}
        url = f"{BRScraper.raw_fundamentals_url_base}{link},Q"
        return self._get_indicators_from_link_and_positions(
            indicators_name_and_locations, url
        )

    def get_balance(self, link: str):
        indicators_name_and_locations = {"Book value": 17}
    ...
    def _get_indicators_from_link_and_positions(
        self, name_and_loc, url
    ) -> pd.DataFrame:
        ...
        indicator_rows = [
            tr[location].find_all("td")[1:-1] for location in name_and_loc.values()
        ]

        for i in range(0, len(seasons) - 1):
            season = seasons[i]
            indicators.loc[season] = [
                float(self._fetch_one_indicator_from_html(row[i]))
