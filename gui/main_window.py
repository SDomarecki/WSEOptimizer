...
from shared.config import read_config, Config
...
class WSEOapp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        df = pd.read_csv('../../database/stooq_source/' + Config.last_used_ticker + '_d.csv', delimiter=';')
        print(df)
    ...
    def get_technical(self, ticker):
        # import pandas as pd
        # reader = pd.read_csv('database/stooq_source/' + ticker + '_d.csv', header=None)
        # print(reader)

        import csv
        stockDays = collections.OrderedDict()

        with open('../../database/stooq_source/' + ticker + '_d.csv') as csv_data:
